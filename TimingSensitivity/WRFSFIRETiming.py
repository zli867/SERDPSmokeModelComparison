...
wrf_sfire_filenames = {
    "HYSPLIT": "/Volumes/PubData/ModelComparisons/Sensitivity/Timing/hysplit/wrfout_d01_2022-04-23_00:00:00",
    "Report": "/Volumes/PubData/ModelComparisons/Sensitivity/Timing/report/wrfout_d01_2022-04-23_00:00:00",
    "GOES-16": "/Volumes/DataStorage/SERDP/data/SFIRE/%s/wrfout_d01_%s_00:00:00" % (select_date.strftime("%Y%m%d"), select_date.strftime("%Y-%m-%d"))
}
...
for monitor_name in monitor_locations.keys():
    ...
    # plot figure
    # WRF-SFIRE
    for start_timing in wrf_sfire_filenames.keys():
        sfire_filename = wrf_sfire_filenames[start_timing]
        wrf_sfire_ds = nc.Dataset(sfire_filename)
        wrf_conc = conc_at_obs_WRF(wrf_sfire_ds, "tr17_2", current_lon, current_lat)
        ax.plot(wrf_conc["time"], wrf_conc["conc"], label=start_timing)
