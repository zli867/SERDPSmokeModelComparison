...
from Concentration.ExtractConcentration import conc_at_obs_CMAQ, conc_at_obs_WRF
...
for plume_scheme in plume_schemes:
    CMAQ_filenames = {
        "HYSPLIT": "/Volumes/PubData/ModelComparisons/Sensitivity/Timing/CMAQ/Combined_3D_CCTM_%s_FtBn1_%s_hysplit_Nudged_noninterp_USFS_updated.nc" % (datetime.strftime(select_date, "%Y%m%d"), plume_scheme),
        "Report": "/Volumes/PubData/ModelComparisons/Sensitivity/Timing/CMAQ/Combined_3D_CCTM_%s_FtBn1_%s_report_Nudged_noninterp_USFS_updated.nc" % (datetime.strftime(select_date, "%Y%m%d"), plume_scheme),
        "GOES-16": "/Volumes/DataStorage/SERDP/data/CMAQ/Combined_3D_CCTM_%s_FtBn1_%s_Nudged_noninterp_USFS_updated.nc" % (datetime.strftime(select_date, "%Y%m%d"), plume_scheme)
    }
    monitor_idx = 1
    for monitor_name in monitor_locations.keys():
        ...
        # plot figure
        # WRF-SFIRE
        for start_timing in CMAQ_filenames.keys():
            cmaq_filename = CMAQ_filenames[start_timing]
            cmaq_ds = nc.Dataset(cmaq_filename)
            cmaq_conc = conc_at_obs_CMAQ(cmaq_ds, "PM25_TOT", current_lon, current_lat)
            ax.plot(cmaq_conc["time"], cmaq_conc["conc"], label=start_timing)
        ...
        plt.title(monitor_name + " " + plume_scheme, fontsize=16)
